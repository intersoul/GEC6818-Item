...
int get_xy(int touch_fd, int *x, int *y, int **p)
{
    // 读取触摸屏信息
...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        int position = get_xy(touch_fd, &x, &y, &p);
        // 第二步：显示图片
        // 如果是点击事件
        // 如果是点击并拖动事件
        // 如果是点击并拖动事件
        ...
        {
            // 在当前点击位置显示图片
            show_pic_any("./zjd.bmp", p, x, y);
        }
        // else if (position == 0) // 点击事件处理（如果需要的话）
        // {
        //     // 你可以在这里添加对点击事件的特殊处理
        // }
        // else
        // {
        //     // 根据滑动方向打印信息，这部分可以根据需求调整
        //     if (position == 1)
        //         printf("向上划动\n");
        //     else if (position == 2)
        //         printf("向下划动\n");
        ...
        // else
        // {
        //     for (int y_offset = 0; y_offset < 480; y_offset++)
        //     {
        //         for (int x_offset = 0; x_offset < 800; x_offset++)
        //             *(p + y_offset * 800 + x_offset) = 0x0000ff00; // 颜色
        //     }
